...
lr_warmup_steps=$(yq -e '.lr_warmup_steps' config.yaml)
weight_decay=$(yq -e '.weight_decay' config.yaml)
clip_grad=$(yq -e '.clip_grad' config.yaml)
...
mp_size=$(yq -e '.mp_size' config.yaml)
pp_size=$(yq -e '.pp_size' config.yaml)

log_interval=$(yq -e '.log_interval' config.yaml)
eval_iters=$(yq -e '.eval_iters' config.yaml)
eval_interval=$(yq -e '.eval_interval' config.yaml)
num_save=$(yq -e '.num_save' config.yaml)
save_interval=$(yq -e '.save_interval' config.yaml)

train_tokens=$(yq -e '.train_tokens' config.yaml)
lr_decay_tokens=$(yq -e '.lr_decay_tokens' config.yaml)
lr_warmup_tokens_in_million=$(yq -e '.lr_warmup_tokens_in_million' config.yaml)
